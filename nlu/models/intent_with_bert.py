...
from data_utils.nlu_intent_postprocess import recover_intent
...
class IntentWithBertPredictor():
    """NLU Intent Classification with Bert 预测器"""

    def __init__(self, config_file, best_model_path):
        ...
        intent_vocab = json.load(
            open(
                "../data/nlu_intent_data/intent_vocab.json",
                encoding="utf-8",
            )
        )
...
if __name__ == "__main__":
    nlu = IntentWithBertPredictor("../config/crosswoz_all_context_nlu_intent.json", "../output/pytorch-intent-with-bert_policy.pt")
